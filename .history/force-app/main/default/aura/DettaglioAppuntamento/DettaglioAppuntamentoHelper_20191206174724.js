({
        ...
        action.setCallback(this, function (response) {
            if (response.getState() == "SUCCESS") {
                ...
                if (resultat.erreur==false) {
                    var dEvent=resultat.detailEvent;
                    this.showToast("Appuntamento modificato con sucesso","success");
                    component.set("v.detailEvent",resultat.detailEvent);
                    component.set("v.isOpen", false);
                    component.set("v.ModifyEvent",false);

                    if(dEvent.Stato_Azione__c=='GAFissato' || dEvent.Stato_Azione__c=='Appuntamento modificato'){
                        component.set('v.showEsito',false);
                    }

                    var urlEvent = $A.get("e.force:navigateToURL");
                    urlEvent.setParams({
                        "url": "https://compass--dev3.lightning.force.com/lightning/o/Account/list?filterName=Recent"
                    });
                    urlEvent.fire();
                } else {
    ...
    uploadRiepilogo : function(component,event,helper) { 
        ...
        var  fotocopieAllegare=component.get('v.fotocopieAllegare');
        var DataglioProductValues = component.get('v.DataglioProductValues');
        var message="message";
	 	var detailEvent = component.get("v.detailEvent");
        let vfUrl = '/apex/RiepilogoAppuntamento?data=' + JSON.stringify(detailEvent)+'&message='+message+'&fotocopieAllegare='+fotocopieAllegare+'&filialeEvent='+JSON.stringify(filialeEvent)+'&DataglioProductValues='+JSON.stringify(DataglioProductValues);
    ...
    getValueMotivazione : function (component,event,helper){
        ...
        if(nuovoStato=='Non presentato/Annullato'){
            ...
            this.aggiornaAppuntamento(component,event,helper);
        }else{
            component.set("v.ModifyEvent",true);
        }
    },
    ...
    getValueFotocopieAllegare : function(component){
        ...
        var detailEvent= component.get("v.detailEvent");
        if(detailEvent.hasOwnProperty('Product__r')){
            var pruduct=detailEvent.Product__r;
            if(pruduct.hasOwnProperty('RSS_External_Id__c')){
