({
        ...
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                ...
                if (!resultat.erreur) {
                    ...
                    var cliente =resultat.account;
                    if(cliente.Id!=null){
                   
                        var nom=component.get("v.cliente.FirstName");
                        if(nom==null){
                            component.set('v.isValueNome',false);
                        }
                        var cognom=component.get("v.cliente.LastName");
                        if(cognom==null){
                            component.set('v.isValueCognome',false);
                        }
                        if(cliente.Provincia_Nascita__pc==null){
                            component.set('v.isValueProvincia',false);
                            this.getProvinceAndComune(component);
                        }
                        if(cliente.Luogo_Nascita__pc==null){
                            component.set('v.isValueComune',false);
                        }
                        if(cliente.Sesso__pc==null){
                            component.set('v.isValueSesso',false);
                        }
                        if(cliente.Data_Nascita__c==null){
                            component.set('v.isValueDateNacita',false);
                            this.crontrolleDataNascita(component);
                        }
                        if(cliente.Codice_Fiscale__pc==null){
                            component.set('v.isValueCF',false);
                            this.getCatasla(component,cliente.Luogo_Nascita__pc);
                        }
    ...
    choiceProdottoDettaglio : function(component,event,helper){
        var testCondition= document.getElementById("checkbox-unique-id-71");
        var productDettaglio;
        var selectPI = document.getElementById("ProductDettaglio");
            if(selectPI.selectedIndex){
                productDettaglio = selectPI.options[selectPI.selectedIndex].value;
    ...
    calculCodiceFiscale: function (component){
        var isValueCF = component.get('v.isValueCF');
        if(isValueCF==false){
    ...
    continuCreaAppuntamento1: function (component,event,helper){
        ...
        var telefonoCellulare =component.get("v.cliente.PersonMobilePhone");
        ...
        if (!phone1 && !telefonoCellulare) {
            ...
            console.log('ici');
        } else if(((phone1==null||phone1.length < 5 || phone1[0] !== '0')&&(telefonoCellulare!=null&&telefonoCellulare.length >=6 && telefonoCellulare[0] == '3'))
               ||((phone1!=null&&phone1.length >= 5 && phone1[0] == '0')&&(telefonoCellulare==null||telefonoCellulare.length <6 || telefonoCellulare[0] !== '3'))){
    ...
    continuCreaAppuntamento3: function (component,event,helper) {
        ...
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                resultat = response.getReturnValue();
                console.log('### newEvent ', JSON.stringify(resultat.newEvent));
                console.log('### resultat ', JSON.stringify(resultat));
                if (resultat) {
                    if(resultat.prodotto==false){
                        this.showToast("Prodotto non valorizzato","error");
                    }else if(resultat.existCLiente==true){
                        this.showToast("il cliente ha già un appuntamento","error");
                        //this.showToast(resultat.messageExistClient,"error");
                        component.set('v.openmodel',false);
                    }else if(resultat.disponibilita==false){
                        this.showToast("Nessun desk disponibile","error");
                        component.set('v.openmodel',false);
                    }else if(resultat.calenarExist==false){
                        this.showToast("Il calendario "+resultat.calenarName+" non é disponibile","error");
    ...
    getValueCliente: function(component) {
        ...
        var testCondition= document.getElementById("checkbox-unique-id-71");
            ...
            var isValueProvincia = component.get('v.isValueProvincia');
            var isValueComune = component.get('v.isValueComune');
            if(isValueProvincia==false){
                    //get province 
                selectPN = document.getElementById("privinceNascita");
                if(selectPN.selectedIndex){
                    privinceNascita = selectPN.options[selectPN.selectedIndex].value;
            ...
            if(selectTO.selectedIndex){
                ...
                newActivity.Tipo_Occupazione__c='Autonomo'
            }
            //get sesso 
    ...
    getCatasla : function(component, comune){
        //getCatastalByComune///comune
        var action = component.get('c.getCatastalByComune'); 
        var resultat;
        action.setParams({ "comune": comune}); 
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                resultat = response.getReturnValue();
                console.log('### resultat ', JSON.stringify(resultat));
                if (resultat) {
                    if(resultat.erreur==false){
                        component.set("v.catastale",resultat.comune);
                    }else{
                    }
                } else {
                }
